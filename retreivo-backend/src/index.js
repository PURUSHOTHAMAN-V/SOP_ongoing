  ...
  try {
    ...
    const claimsWithFraudScores = await Promise.all(
      allClaims.map(async (claim) => {
        try {
          // Call ML service for enhanced fraud detection
          const mlResponse = await fetch(`${process.env.ML_SERVICE_URL || 'http://localhost:5002'}/detect-fraud`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              item_details: {
          ...
          let fraudScore = 50; // Default moderate risk
          let fraudIndicators = [];
          
          if (mlResponse.ok) {
            const mlData = await mlResponse.json();
            fraudScore = mlData.fraud_score || 50;
